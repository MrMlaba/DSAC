...
export default async function EntityCompliancePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const user = await requireDsacUser();
  const { id } = await params;
  const { fy } = await searchParams;
  const ctx = await getEntityContext(user, id, fy).catch(() => null);
  if (!ctx) notFound();
  return <ComplianceView ctx={ctx} />;
}
